...
Update 循环是 Unity 性能问题的**第一大源头**。本节讲三件事：每帧别分配、别重复算、别每帧都跑。
...
// 说明：Unity 2022.2+ 可使用 FindObjectsOfTypeInactive<T>(List<T>) 避免GC
// 但不建议在 Update 中频繁查找对象，最好通过管理器模式维护引用
```
...
> **何时自己实现 vs 用 ObjectPool<T>**：除非 Unity 2020 或有特殊需求，**优先用 `UnityEngine.Pool.ObjectPool<T>`**。自己实现容易出 bug（Stack 不线程安全、maxSize 处理、GameObject 销毁时机等）。
...
### 8.2 跨版本选型（协程 vs Awaitable vs UniTask）

- ✅ **Unity 6+**：简单延时优先使用 `Awaitable`（零分配）而非协程。
- ✅ **全版本通用**：协程也能用，注意缓存 `WaitForSeconds` 对象。
...
- ✅ **临时小数组用 `Span<T>` + `stackalloc`**：栈上分配，零 GC。
- ✅ 适合处理固定大小缓冲区（音频、几何顶点、网络数据包解析）。
...
> **限制**：`stackalloc` 的数组**不能太大**（通常 < 1KB），否则会栈溢出。
...
- **Unity 6+**：优先使用 `Awaitable` 而非协程（其他版本用协程）
