...
test("Release 来源记录固定仓库版本和完整提交号", async () => {
  ...
  try {
    ...
    const valid = spawnSync(process.execPath, [script, "v0.1.0", sourceCommit, destination], { cwd: repository, encoding: "utf8" });
    assert.equal(valid.status, 0, valid.stderr);
    assert.deepEqual(JSON.parse(await readFile(destination, "utf8")), { schemaVersion: 1, version: "0.1.0", sourceCommit });
    const invalid = spawnSync(process.execPath, [script, "v0.2.0", sourceCommit, destination], { cwd: repository, encoding: "utf8" });
    assert.notEqual(invalid.status, 0);
    assert.match(invalid.stderr, /与 v0\.2\.0 不一致/);
  } finally {
...
test("完整规则源按 Release 布局打包后可校验和读取", async () => {
  ...
  try {
    ...
    await writeFile(path.join(root, "agentrulekit-release.json"), JSON.stringify({ schemaVersion: 1, version: "0.1.0", sourceCommit }));
    ...
    const downloaded = await downloadRulepacks({ version: "0.1.0", assetUrl, digest }, fetcher);
    try {
