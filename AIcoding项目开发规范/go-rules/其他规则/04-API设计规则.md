...
> 🟢 本文件适用于 **HTTP REST API** 和 **gRPC API** 的设计与修改<br>
...
🟡 推荐：内部服务间调用、操作型 API（非 CRUD）用 RPC 风格
...
| 场景 | 用 |
|---|---|
| 资源 CRUD（用户、订单、商品） | REST（标准方法 + 资源路径） |
| 操作/动作（发邮件、导出、转账） | RPC（`POST /rpc/<action>`） |
| 内部微服务间调用 | gRPC（见 § 6） |
| 前端 BFF 层 | 混合：CRUD 用 REST，复杂聚合用 RPC |
...
- **GET 请求参数放 query string**，不用 request body（虽然 HTTP 规范允许，但 Go 的 `net/http` 不解析 GET body）
...
| `time.Time` | 省略零值时间 | ✅ 默认加 `omitempty` |
| 指针类型 `*T` | 省略 nil | ✅ 区分"未传"和"零值"的唯一方式 |
| slice / map | 省略 nil（空 slice `[]` 也省略） | ⚠️ 前端可能需要 `[]` 而非 `null` |
...
- **始终返回 JSON**（`Content-Type: application/json`）
- **列表接口必须返回分页信息**（`totalCount` + `page` + `pageSize`）
- **不要返回 Go 的 nil 切片为 `null`**——初始化为空切片 `[]T{}` 或确保 JSON tag 输出 `[]`
...
> **游标 vs 偏移**：数据量大（> 10000 条）且频繁翻页时，游标分页远优于偏移分页（避免 OFFSET 扫描）。
...
type UserV2 struct {
    ID    int    `json:"id"`
    Name  string `json:"name"`
    Email string `json:"email,omitempty"` // v2 新增，omitempty 保证兼容
}
```
...
func createOrder(w http.ResponseWriter, r *http.Request) {
    ...
    if err := validate.Struct(req); err != nil { // 格式校验
...
🔴 必加约束：对外 HTTP API **必须**有 OpenAPI 文档
