...
> 🟢 本文件适用于 TypeScript 独有的语法规范（类型注解/比较运算/装饰器等）。<br>
> 🔴 不适用：业务逻辑实现、UI 样式设计
...
function parse(input: unknown): User {
  ...
  return JSON.parse(input) as User;                  // 已收窄
}
```
...
**不要用 legacy decorators**（旧实验性 API，已不推荐）。
...
{
  "compilerOptions": {
    "experimentalDecorators": false                   // 用 TC39 新版
  }
}
```
...
**具名优先**（与 [01-代码编写规则](../../js_rulers/其他规则/01-代码编写规则.md) 一致）：
