...
🔴 **永远用 TextMeshPro（TMP）替代老 Text 组件**。TMP 用 SDF 字体渲染，可任意缩放保持清晰。
...
**字体设置**：
- 🔴 项目首次使用 TMP 时导入 `TMP Essentials`（Window → TextMeshPro → Import TMP Essential Resources）
- 🟡 中文字体推荐用 **动态字体**（Dynamic Font Asset）支持任意字符
- 🟡 静态字符（如固定 UI 文本）可用 **静态字体**（Static Font Asset）性能更好
...
🔴 **UI 贴图导入设置**（`Texture Importer`）：

| 设置 | 推荐值 | 原因 |
|------|--------|------|
| `Texture Type` | `Sprite (2D and UI)` | 专为 UI 设计 |
| `Sprite Mode` | `Single` | 单图用 Single；多图用 Multiple |
| `Pixels Per Unit` | `100`（2D）/ `50`（UI） | 决定 1 米对应多少像素 |
| `Filter Mode` | `Bilinear` | 缩放平滑（像素图用 Point）|
| `Compression` | `Compressed` | 节省内存 |
| `Max Size` | `2048`（图标）/ `4096`（背景） | 限制大图 |
| `Format` | `RGBA 32 bit`（有透明）/ `RGB 24 bit`（无透明） | 节省内存 |
| `Wrap Mode` | `Clamp` | 防止边缘重复 |

## 9. C# 端用法

### 9.1 [SerializeField] 引用 UI 组件

🔴 **所有 UI 组件引用都用 `[SerializeField] private`**，不要 `public` 也不要 `GetComponent` 每次拿。
...
private void OnButtonClicked()
{
    var btn = GetComponent<Button>();  // 每帧查找
    btn.onClick.AddListener(...);
}

// ✅ 对：[SerializeField] private
...
- 🔴 UI 类 `MonoBehaviour` 在 `Awake` 缓存所有 UI 组件引用
- 🟡 **动态生成的 UI**（如 ListView 子项） 在 `OnEnable` 缓存
