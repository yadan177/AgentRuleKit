...
> 🟢 本文件适用于 Unity 项目的性能调优（GC/DrawCall/物理/渲染）。<br>
...
- 🔴 禁止用 `.material`（创建材质实例）；用 `MaterialPropertyBlock`。
- ❌ 除非必要，避免运行时切换材质。
...
// ✅ 好 —— 使用 sharedMaterial（不创建实例）
private Renderer m_renderer;
private void SetSharedColor(Color color)
{
    m_renderer.sharedMaterial.color = color;  // 修改共享材质，所有对象受影响
}
```
