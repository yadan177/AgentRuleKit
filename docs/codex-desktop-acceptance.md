...
| 当前个人 Codex 环境 | 2026-09-25 在本仓库运行 `codex-cli 0.142.0` 的 `codex plugin list`、`codex plugin marketplace list`，均未列出 `agent-rule-kit` 或 `agentrulekit` 市场；这只是 CLI 可见状态，不能代替桌面端检查 | 尚未在当前个人环境确认安装 |
