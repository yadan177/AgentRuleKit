...
本清单用于补齐[首版开发计划](development-plan.md)的 Codex 桌面端证据。CLI 隔离安装、插件格式校验与 Hook 脚本模拟运行已经通过，但它们不能证明桌面端已发现插件、实际调用 Skill 或获得用户对 Hook 的信任。仓库当前保持 Private，不创建真实 Release 来冒充这项验收。
