...
| C. 公开分发 | 仓库公开、安全审计、GitHub Release 规则包、npm 首次发布、外部干净环境复测 | 尚未发布 | 远程 `init/diff/update` 和插件安装端到端通过 |
| D. 后续扩展 | Cursor、TRAE、Qoder 适配器；可选的更新 PR 自动化 | 不在首版范围 | 每个适配器独立能力矩阵与跨平台测试 |

## 发布前必须过的门槛

- CI 在 Linux、macOS、Windows 的 Node.js 22/24 上通过，并检查 npm 包内容与版本一致性。
...
当前实现细节见 [安装与更新](installation.md)，发布操作见 [发布说明](releasing.md)。
