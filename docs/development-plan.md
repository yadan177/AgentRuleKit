...
| B. Codex 接入 | 规则入口、四个 Skills、仓库插件市场、可信任后启用的会话后台检查 | 本地代码已实现；隔离 CLI 安装和四个 Skills 发现已通过，Hook 脚本在真实 `0.1.0 → 0.1.1` Release 上发出提醒且不写项目。桌面端交互、Hook 信任与实际会话调度仍待验收 | 插件/Skill 校验；Hook 首次请求、每日缓存与锁变刷新；隔离 CLI 安装及提示输入检查；桌面端验收 |
| C. 公开分发 | 仓库公开、安全审计、GitHub Release 规则包、npm 首次发布、外部干净环境复测 | `v0.1.0` 与 `v0.1.1` Release、npm 包均已发布；`0.1.1` 通过 GitHub Actions OIDC 发布并附 provenance；npm 独立安装及 Go、TypeScript、Unity 的真实跨版本规则更新均通过。逐文件内容权利和人工隐私审查仍待补齐 | 真实远程安装、npm 独立安装和后续版本更新均通过 |
...
- npm 包 `agentrulekit` 的所有权、首次发布与 GitHub Actions 可信发布者配置已由仓库所有者完成；`0.1.1` 已通过 OIDC 实际发布，不要求向开发代理提供 npm 密码或令牌。
- 已在全新环境从 npm 和 GitHub Release 公开入口安装并完成真实跨版本更新；尚未完成的桌面端验收须单独说明。
