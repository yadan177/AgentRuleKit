...
| C. 公开分发 | 仓库公开、安全审计、GitHub Release 规则包、npm 首次发布、外部干净环境复测 | 仓库保持私有；模拟 Release 的 CLI 命令级远程闭环已通过，真实 Release 与外部干净环境仍待内容授权和发布验收 | 真实远程 `init/check/diff/update/validate` 和插件安装端到端通过 |
