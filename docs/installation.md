...
从下一版发布开始，如果规则源包含根目录的普通文本 `LICENSE`，安装器还会把它复制为业务项目中的 `.agent-rules/LICENSE`，记录在锁文件并纳入后续差异预览和完整性校验。已有同名未知文件时会停止，不会覆盖。已发布的 `v0.1.0`、`v0.1.1` GitHub Release 不含该文件，不能因此补出一份许可文本；此文件也不能代替对各规则正文中第三方材料的逐项权利核对。
