...
5. 在 npm 账户上完成 `agentrulekit` 首次发布所需的所有权与认证设置，并将 `publish-npm.yml` 配置为可信发布者。工作流要求 npm CLI 11.5.1+、Node.js 22.14+ 和 GitHub `id-token: write`。之后从对应标签手动触发 npm 发布；工作流会在发布前再次核对同版本 GitHub Release 的规则包摘要和来源提交，Release 缺失、资产损坏或提交不符都会阻断 npm 发布。首次发布前不能假定 npm 包名已经归本项目所有。
