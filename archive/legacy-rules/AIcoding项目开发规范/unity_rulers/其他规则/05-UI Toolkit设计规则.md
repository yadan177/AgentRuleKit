...
| CSS 变量 | `--color: red;` 可写在选择器中 | USS 自定义属性可继承；全局 token 常放 `:root`，局部 token 可按目标 Unity 版本在作用域选择器中验证 |
| `@media` 查询 | ✅ | ❌ 不支持 |
| `@import` | ✅ | 支持范围和路径解析随 Unity 版本核对；也可用 UXML `<Style src="..."/>` 或 C# 添加 StyleSheet |
| 字体相对单位 | `em`、`rem` | ❌ — 改用 `px` |
...
`picking-mode` 是 UXML 属性，`pickingMode` 是 C# 属性，不是 USS 样式属性：

```xml
<!-- 忽略指针事件，让命中测试继续到下层元素 -->
<ui:VisualElement name="decoration" picking-mode="Ignore" />
```

```csharp
decoration.pickingMode = PickingMode.Ignore;
```
...
全局设计 token 常声明在 `:root` 中并由后代继承。需要局部主题时，可在目标 Unity 版本验证后把自定义属性声明在容器 class 上；不要把 `:root` 写成唯一合法位置。
...
用于在多个内容面板间切换的 Tab 界面。USS 选择器参见 [TabView 与 Tab 样式](#16-tabview-与-tab-样式)。
...
{
    // 调用当前界面已定义的业务处理；不要只写 Debug.Log 作为最终行为。
}
```
...
// 指针事件：按当前界面的高亮/提示策略处理。
element.RegisterCallback<PointerEnterEvent>(evt => { /* 更新悬停状态 */ });
element.RegisterCallback<PointerLeaveEvent>(evt => { /* 清理悬停状态 */ });
...
3. 检查没有覆盖元素拦截指针事件
4. 检查 UXML `picking-mode="Position"` 或 C# `pickingMode = PickingMode.Position`，不要把它写进 USS
